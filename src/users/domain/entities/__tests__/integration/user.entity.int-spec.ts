...
describe('UserEntity integration tests', () => {
  ...
  describe('Update method', () => {
    it('Should throw an error when update a user with invalid name', () => {
      ...
      expect(() => entity.update({ name: null as any })).toThrow(
    ...
    it('Should throw an error when update a user with invalid email', () => {
      const entity = new UserEntity(UserDataBuilder({}));
      expect(() => entity.update({ email: null as any })).toThrow(
        EntityValidationError,
      );
      ...
      expect(() => entity.update({ email: 'a'.repeat(256) })).toThrow(
        EntityValidationError,
      );
    });
    ...
    it('Should a valid user', () => {
      ...
      entity.update({ name: 'other name', email: 'email@email.com' });
    });
  });
